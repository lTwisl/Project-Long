...
public class WeatherSystemEditor : Editor
{
    ...
    private void CreateTimeInfoSection(VisualElement container)
    {
        ...
        CreateReadOnlyField("Начало текущей погоды:",
            WorldTime.Instance != null ?
            WorldTime.Instance.GetFormattedTime(system.TimeStartCurrentWeather) :
            system.TimeStartCurrentWeather.ToString(), timeSection);

        CreateReadOnlyField("Конец текущей погоды:",
            WorldTime.Instance != null ?
            WorldTime.Instance.GetFormattedTime(system.TimeEndCurrentWeather) :
            system.TimeEndCurrentWeather.ToString(), timeSection);
    ...
    {
        if (!weatherSystem.CheckHasTransition() || WorldTime.Instance == null) return 0;
