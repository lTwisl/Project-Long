...
public enum WeatherIdentifires
{
    ...
    Overcast = 3,       // ☀️ - Сильно пасмурно
    Foggy = 4,          // ☀️☀️ - Туманно
    Rainу = 5,          // ☀️ - Дождливо
    Sleet = 6,          // ❆ - Слякоть = снег + дождь
    Shower = 7,         // ❆ - Ливень
    SunnySnow = 8,      // ❆ - Легкий снег с солнцем
    OvercastSnow = 9,   // ❆ - Легкий снег пасмурно
    Snowfall = 10,      // ❆❆ - Снегопад
    SnowStorm = 11,     // ❆❆❆ - Снежная буря с молниями
    AluminumRain = 12,  // ☣ - Аэрозольный смог (добавляет зараженность)
    MetallWind = 13,    // ☣ - Металлический ветер (добавляет зараженность + снимает здоровье)
    AerosolSmog = 14,   // ☣☣ - Аллюминевый дождь (добавляет зараженность)
    ToxicIceStorm = 15  // ☣☣☣ - Ледяной шторм Хлад-9 (добавляет зараженность + снимает здоровье)
}
...
public class WeatherProfile : ScriptableObject
{
    ...
    [field: SerializeField] public Material NearVolumFogMat { get; private set; }
    [field: SerializeField] public Material FarVolumFogMat { get; private set; }
